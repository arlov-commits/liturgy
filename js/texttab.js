...
(function (root) {
  "use strict";
  const { EditorView, EditorState, basicSetup, StreamLanguage, HighlightStyle, syntaxHighlighting, tags } = CM;
  ...
  function build(box, select, sections, onChange, extensions = []) {
    ...
    const stateFor = (s) => states[s.name] || (states[s.name] = EditorState.create({
      doc: s.text,
      extensions: [
        basicSetup, EditorView.lineWrapping, liturgy, syntaxHighlighting(colours), theme, ...extensions,
