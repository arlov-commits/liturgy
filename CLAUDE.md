...
- **The editor never writes `text/`** — that's the original (baseline). Edits are a parallel edition in
  ...
  Its entries are written out (`[contents]` … `[/contents]`, one `title [page of chapter]` line each) and kept in step with
...
| Binding (Settings → Binding) | `--binding`: `perfect` (4 a side on letter, cut, glued), `in-order` (pages in reading order, one per sheet — prints the pages as laid out), `signatures` (below) | the Print panel shows the steps for the one chosen |
| Folded signatures (Settings → Binding) | our own `impose.js signatures()`, following **bookbinder-js** (MPL-2.0; read, not copied) | letter sheets landscape, 2 pages a side, flip on short edge; `signaturePlan`: at most 8 sheets (32 pages) per signature, split evenly (sizes differ by ≤ 1 sheet), blanks at the end; one signature > 32 pages is warned about. Print panel shows the split. |
